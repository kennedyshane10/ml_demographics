...
## Some metrics for second model with larger lambda 
```{r}
...
We then do the same using the second model fitted with larger lambda
```{r}
...
For the second model, where we deliberately chose the lambda tuning parameter to be high, fewer variables are selected. This model is obviously easier to interpret however the deviance function is higher. 
